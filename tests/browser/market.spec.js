...
test('浏览器双用户发布审核收藏联系已售闭环', async ({ browser }) => {
  ...
  await buyer.getByRole('button', { name: '关闭', exact: true }).click(); await buyer.locator('[data-nav=favorites]').click(); await expect(buyer.locator('.product-card').filter({ hasText: title })).toBeVisible();
...
test('桌面和手机视口无横向溢出，搜索空状态和分类可用', async ({ page }) => {
  ...
  await expect(page.getByText('暂时没有找到相关好物')).toBeVisible();
