...
function loginDialog() {
  if (user) { showDialog(`<div class="login-content"><span class="brand-mark">盒</span><h2>${esc(user.name)}</h2><p class="muted">把闲置分享给需要它的人。</p><button class="secondary" data-action="logout">退出登录</button></div>`); return; }
  showDialog(`<div class="login-content"><span class="brand-mark">盒</span><h2 class="dialog-heading">欢迎来到盒闲</h2><p class="muted">登录后，发布闲置、收藏好物，<br>与同校同学取得联系。</p>${config.demo ? '<button class="primary" data-action="login" data-account="seller">以小林同学体验 · 卖家</button><button class="secondary" data-action="login" data-account="buyer">以小陈同学体验 · 买家</button><p class="muted">仅本机演示身份，不代表微信或校园认证。</p>' : '<p>请在微信小程序中使用微信登录。</p>'}<a href="/policy.html" class="text-button" target="_blank">使用须知与隐私说明</a></div>`);
}
...
async function navigate() {
  ...
  route = ['home', 'mine', 'favorites', 'profile', 'lost', 'messages'].includes(hash) ? hash : 'home'; await setRoute();
}
async function setRoute() {
  document.querySelectorAll('[data-nav]').forEach(el => el.classList.toggle('active', el.dataset.nav === (['mine','favorites'].includes(route) ? 'profile' : route)));
  closeSearch();
  $('#home-intro').hidden = route !== 'home'; $('#filter-row').hidden = route !== 'home';
  $('#profile-panel').hidden = route !== 'profile';
  const placeholder = ['lost','messages'].includes(route);
  $('#placeholder-panel').hidden = !placeholder;
  $('#market-section').hidden = ['profile','lost','messages'].includes(route);
  if (placeholder) { $('#placeholder-title').textContent = route === 'lost' ? '失物招领' : '消息'; $('#placeholder-copy').textContent = route === 'lost' ? '失物招领尚未开放，请通过学校现有渠道发布或寻找失物。' : '站内聊天尚未开放。请在商品详情中登录查看卖家联系方式，线下沟通交易。'; }
  if ($('#market-section').hidden) { ++renderId; return; }
...
$('#search-form').addEventListener('submit', event => { event.preventDefault(); query = $('#search').value.trim(); closeSearch(); if(route !== 'home') location.hash='home'; else renderFeed(); });
...
let suggestionId=0;
function closeSearch(){suggestionId++;$('.app-header').classList.remove('searching');$('#search-suggestions').hidden=true;$('#cancel-search').hidden=true;$('#search').setAttribute('aria-expanded','false');}
async function liveSearch(){const current=++suggestionId;$('.app-header').classList.add('searching');$('#cancel-search').hidden=false;$('#search-suggestions').hidden=false;$('#search').setAttribute('aria-expanded','true');$('#search-suggestions').textContent='正在查找…';try{const data=await api('/products?q='+encodeURIComponent($('#search').value.trim())+'&page=1');if(current!==suggestionId)return;$('#search-suggestions').innerHTML=data.items.slice(0,8).map(p=>'<a href="#detail/'+encodeURIComponent(p.id)+'">'+esc(p.title)+' · ¥'+esc(p.price)+'</a>').join('')||'<p>暂时没有找到相关好物</p>';}catch(error){if(current===suggestionId)$('#search-suggestions').textContent=error.message;}}
$('#search').addEventListener('focus',liveSearch);$('#search').addEventListener('input',liveSearch);$('#search').addEventListener('keydown',e=>{if(e.key==='Escape')closeSearch();});$('#cancel-search').addEventListener('click',closeSearch);$('#search-suggestions').addEventListener('click',e=>{if(e.target.closest('a'))closeSearch();});document.addEventListener('click',e=>{if(!e.target.closest('.app-header'))closeSearch();});
document.addEventListener('keydown',e=>{if(e.target.matches('.upload-button')&&['Enter',' '].includes(e.key)){e.preventDefault();$('#photo-input')?.click();}});
const track=$('.carousel-track'),slides=[...track.children].filter(el=>!el.hasAttribute('data-carousel-clone'));track.querySelectorAll('[data-carousel-clone]').forEach(el=>el.remove());let slide=0,paused=false;const first=slides[0].cloneNode(true);first.setAttribute('aria-hidden','true');track.append(first);let resetSlide;
function moveSlide(index){clearTimeout(resetSlide);slide=index;track.style.transition='';track.style.transform='translateX(-'+(slide*100)+'%)';document.querySelectorAll('button[data-slide]').forEach((button,i)=>{button.classList.toggle('active',i===slide%3);button.setAttribute('aria-current',String(i===slide%3));});if(slide===3)resetSlide=setTimeout(()=>{track.style.transition='none';track.style.transform='translateX(0)';slide=0;},650);}
document.querySelectorAll('button[data-slide]').forEach(button=>button.addEventListener('click',()=>moveSlide(Number(button.dataset.slide))));$('.carousel-pause').addEventListener('click',()=>{paused=!paused;$('.carousel-pause').textContent=paused?'▶':'Ⅱ';$('.carousel-pause').setAttribute('aria-label',paused?'继续轮播':'暂停轮播');});moveSlide(0);setInterval(()=>{if(!paused&&!document.hidden&&route==='home'&&!$('#dialog').open&&$('#search-suggestions').hidden)moveSlide(slide+1);},5000);
